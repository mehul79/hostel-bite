...
export default function Products() {
  ...
  useEffect(() => {
    if (
      selectedTag !== "All" &&
      tagOptions.length > 1 &&
      !tagOptions.includes(selectedTag)
    ) {
      setSelectedTag("All");
    }
  ...
  const handleVote = async (productId: string) => {
    try {
      const response = await api.post<{ score: number }>(
        ENDPOINTS.PRODUCTS.VOTE(productId),
        { vote: 1 },
      );
      if (response.data) {
        setProducts((prev) =>
    ...
    <div className="container-wide py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Discover delicious food from shops in your hostel
        </p>
      </div>
